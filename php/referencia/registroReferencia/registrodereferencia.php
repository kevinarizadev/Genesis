...
function consultadeRefrencias()
{
	...
	require_once('../../config/dbcon.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_REFERENCIA.p_consulta_referencias_activas(:v_pgrupo,:v_pestado,:v_json_row);end;');
	$clob = oci_new_descriptor($c, OCI_D_LOB);
...
function obtenerNombreipsReceptora()
{
	...
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_REFERENCIA.p_obtener_ips_receptora(:v_pnit,:v_pnumero,:v_json_row); end;');
	oci_bind_by_name($consulta, ':v_pnit', $ips);
