...
function Actualizar_Diag_Clase()
{
  require_once('../../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN PQ_GENESIS_GESTION_ACGS.P_ACTUALIZA_CLASE_SINIESTRO(:v_pclase_correcta,:v_pdiagnostico,:v_pradicado,:v_pjson_row_out); end;');
...
function Obt_Registros_Inde()
{
  require_once('../../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN PQ_GENESIS_GESTION_ACGS.P_LISTAR_REGISTROS_INDETERMINADOS(:v_presponsable,:v_pjson_row_out); end;');
  oci_bind_by_name($consulta, ':v_presponsable', $request->Cedula);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_pjson_row_out', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
...
function Obt_Detalle_Municipio()
{
  require_once('../../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN PQ_GENESIS_GESTION_ACGS.P_DETALLE_MUNICIPIO(:v_presponsable,:v_seccional,:v_pconcepto,:v_pjson_row_out); end;');
  oci_bind_by_name($consulta, ':v_presponsable', $request->Cedula);
  oci_bind_by_name($consulta, ':v_seccional', $request->Sec);
  oci_bind_by_name($consulta, ':v_pconcepto', $request->Coh);
...
function obtenerClasificacion()
{
  ...
  $consulta = oci_parse($c, 'BEGIN PQ_GENESIS_GESTION_ACGS.P_CONSULTAR_CLASE_CONCEPTO(:v_cohorte,:v_response); end;');
