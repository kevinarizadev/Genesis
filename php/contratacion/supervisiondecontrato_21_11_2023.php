...
function guardarrevisionContrato()
{
	require_once('../../php/config/dbcon.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN pq_genesis_supervision.p_ui_supervision(:v_pdocumento,:v_pnit,:v_pregional,:v_pfecha_visita,
...
{
	require_once('../../php/config/dbcon.php');
	$consulta = oci_parse($c, 'BEGIN pq_genesis_supervision.p_obtener_preguntas(:v_json_row); end;');
	$clob = oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
	oci_execute($consulta, OCI_DEFAULT);
	if (isset($clob)) {
