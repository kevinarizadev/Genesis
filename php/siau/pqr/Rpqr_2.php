...
function P_GUARDA_SEGUIMIENTO()
{
	require_once('../../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_PQR.P_GUARDA_SEGUIMIENTO(:v_pqr,:v_seguimiento,:v_servicio,:v_fecha_seg,:v_observacion,:v_responsable,:v_pjson_row); end;');
	oci_bind_by_name($consulta, ':v_pqr', $request->pqr);
	oci_bind_by_name($consulta, ':v_seguimiento', $request->seguimiento);
...
function p_obtener_exportado_seguimiento()
{
	require_once('../../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_PQR.p_obtener_exportado_seguimiento(:V_RESULT); end;');
	$cursor = oci_new_cursor($c);
	oci_bind_by_name($consulta, ':V_RESULT', $cursor, -1, OCI_B_CURSOR);
	oci_execute($consulta);
	oci_execute($cursor, OCI_DEFAULT);
