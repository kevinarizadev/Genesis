...
function Hoja1_Anular_Siniestros()
{
	require_once('../../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_GESTION_ACGS.P_ANULA_SINIESTRO_NACIONAL(:v_pradicado,:v_presponsable,:v_pobservacion,:v_ptipo,:v_pfecha_retiro,:v_pjson_row); end;');
	...
	oci_bind_by_name($consulta, ':v_pfecha_retiro', $request->Fecha_Retiro);
	$clob = oci_new_descriptor($c, OCI_D_LOB);
