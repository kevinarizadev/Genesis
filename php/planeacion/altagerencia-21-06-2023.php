...
function actualizarCompromiso()
{
    ...
    $insertar = oci_parse($c, 'BEGIN OASIS.PQ_ALTA_GERENCIA.P_AG_UPDATE_COMPROMISOS(
        ...
        :v_json_row); end;');
    $clob = oci_new_descriptor($c, OCI_D_LOB);
    oci_bind_by_name($insertar, ':p_i_compromiso', $request->idCompromiso);
...
function subirFTP()
{
    // require_once('../config/sftp_con_ag.php');
    ...
    // if (is_dir('ftp://wikode:Wikode2023!!@172.20.0.7/' . $path) == TRUE) {
...
function descargaAdjunto()
{
    global $request;
    $fileexists = false;
    // if (file_exists('ftp://wikode:Wikode2023!!@172.20.0.7/' . $request->ruta) == TRUE) {
