...
function subir_archivos()
{
  ...
  for ($i = 0; $i < count($archivos); $i++) {
		$name = $request->numero . '_' .  $archivos[$i]->codigo . '_' . $ubicacion.'.'.$archivos[$i]->ext;
		list(, $archivos[$i]->archivo) = explode(';', $archivos[$i]->archivo); // Proceso para traer el Base64
		list(, $archivos[$i]->archivo) = explode(',', $archivos[$i]->archivo); // Proceso para traer el Base64
		$base64 = base64_decode($archivos[$i]->archivo); // Proceso para traer el Base64
		file_put_contents('../../temp/'.$name, $base64); // El Base64 lo guardamos como archivo en la carpeta temp
		
		$subio = UploadFile($path, $name);
		if (substr($subio, 0,11) == '/cargue_ftp') {
			array_push($jsonSalida, (object)[
				'ruta' => $subio,
...
require('../sftp_cloud/UploadFile.php');
  ...
  for ($i = 0; $i < count($archivos); $i++) {
    $hora = date('h_i_s');
		$name = $request->numero . '_' .  $archivos[$i]->codigo . '_archivo_actualizado_devolucion_1' . $hora.'.'.$archivos[$i]->ext;
		list(, $archivos[$i]->archivo) = explode(';', $archivos[$i]->archivo); // Proceso para traer el Base64
		list(, $archivos[$i]->archivo) = explode(',', $archivos[$i]->archivo); // Proceso para traer el Base64
		$base64 = base64_decode($archivos[$i]->archivo); // Proceso para traer el Base64
		file_put_contents('../../temp/'.$name, $base64); // El Base64 lo guardamos como archivo en la carpeta temp
...
function p_insertar_prestacion()
{
  ...
  $consulta = oci_parse($c, 'BEGIN PQ_GENESIS_PRESTACIONES.P_INSERTAR_PRESTACION(:v_pubicacion,
                                                                                  ...
                                                                                  :v_tutela,
                                                                                  :v_grupo_servicio,
                                                                                  :v_mod_prestacion_servicio,
                                                                                  :v_incapacidad_retroactiva,
                                                                                  :v_json_row); end;');
  oci_bind_by_name($consulta, ':v_pubicacion', $ubicacion);
  oci_bind_by_name($consulta, ':v_ptipo_doc_afiliado', $request->tipo_documento);
  oci_bind_by_name($consulta, ':v_pafiliado', $request->documento);
  ...
  oci_bind_by_name($consulta,':v_grupo_servicio',$request->grupo_servicio);
  oci_bind_by_name($consulta,':v_mod_prestacion_servicio',$request->mod_prestacion_servicio);
  oci_bind_by_name($consulta,':v_incapacidad_retroactiva',$request->incapacidad_retroactiva);
...
function p_insertar_val_medica()
{
  ...
  oci_bind_by_name($consulta, ':v_pdiagnostico', $request->v_pdiagnostico);
  oci_bind_by_name($consulta, ':v_pespecilidad', $request->v_pespescialidad);
  oci_bind_by_name($consulta,':v_grupo_servicio',$request->v_grupo_servicio);
  oci_bind_by_name($consulta,':v_mod_prestacion_servicio',$request->v_mod_prestacion_servicio);
  oci_bind_by_name($consulta,':v_incapacidad_retroactiva',$request->v_incapacidad_retroactiva);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_res', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
...
function DatosElic(){
  require_once('../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'begin oasis.PQ_GENESIS_PRESTACIONES.P_OBTENER_DATOS_APORTE(:v_tipo_documento,
  :v_numero_documento,
  :v_numero_documento_aportante,
  :v_json_row); end;');
  ...
  oci_bind_by_name($consulta, ':v_numero_documento_aportante', $request->aportante);
  // oci_bind_by_name($consulta, ':v_tipo_cotizante', $request->tipo_cotizante);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
...
// function obtenerdatosincapacidad(){
//   require_once('../config/dbcon_prod.php');
//   global $request;
//   $consulta = oci_parse($c, 'BEGIN oasis.PQ_AUT_PRESTACION.P_OBTENER_FECHAS(:v_tipo_documento,:v_numero_documento,:v_json_row); end;');
//   oci_bind_by_name($consulta, ':v_tipo_documento', $request->tipodocumento);
//   oci_bind_by_name($consulta, ':v_numero_documento', $request->documento);
//   $clob = oci_new_descriptor($c, OCI_D_LOB);
//   oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
...
//   require_once('../config/dbcon_prod.php');
//   global $request;
//   $consulta = oci_parse($c, 'BEGIN oasis.PQ_AUT_PRESTACION.P_CALCULAR_IBC(:v_fecha_inicio,
//                                                                           :v_tipo_documento_afi,
//                                                                           :v_documento_afi,
//                                                                           :v_documento_aportante,
//                                                                           :v_tipo_prestacion,
//                                                                           :v_tipo_cotizante,
//                                                                           :v_json_row); end;');
//   oci_bind_by_name($consulta, ':v_fecha_inicio', $request->fecha_inicio);
//   oci_bind_by_name($consulta, ':v_tipo_documento_afi', $request->tipodocumento);
//   oci_bind_by_name($consulta, ':v_documento_afi', $request->documento);
//   oci_bind_by_name($consulta, ':v_documento_aportante', $request->aportante);
//   oci_bind_by_name($consulta, ':v_tipo_prestacion', $request->tipo_prestacion);
//   oci_bind_by_name($consulta, ':v_tipo_cotizante', $request->tipo_cotizante);
//   $clob = oci_new_descriptor($c, OCI_D_LOB);
//   oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
//   oci_execute($consulta, OCI_DEFAULT);
...
//   oci_close($c);
// }
function OBTENER_DATOS_DEL_CALCULO(){
  require_once('../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_GENESIS_PRESTACIONES.P_valida_salario_variable(:V_PTIPO_DOC,:V_PNUMERO,:v_finicio_incapacidad,:v_documento_aportante,:v_json_row); end;');
  oci_bind_by_name($consulta, ':V_PTIPO_DOC', $request->tipodocumento);
  oci_bind_by_name($consulta, ':V_PNUMERO', $request->documento);
  oci_bind_by_name($consulta, ':v_finicio_incapacidad', $request->fecha_incapacidad);
  oci_bind_by_name($consulta, ':v_documento_aportante', $request->documento_aportante);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
...
function OBTENER_DATOS_DEL_CALCULO_LINCENCIA(){
  require_once('../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_GENESIS_PRESTACIONES.P_CALCULO_VARIABLES_LICENCIAS_BASE40(:V_PTIPO_DOC,:v_pnumero,:v_pfinicio_incapacidad,:v_pdocumento_aportante,:v_pjson_row); end;');
  oci_bind_by_name($consulta, ':V_PTIPO_DOC', $request->tipodocumento);
  oci_bind_by_name($consulta, ':v_pnumero', $request->documento);
  oci_bind_by_name($consulta, ':v_pfinicio_incapacidad', $request->fecha_incapacidad);
  oci_bind_by_name($consulta, ':v_pdocumento_aportante', $request->documento_aportante);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
...
function DATOS_PRORROGA(){
  require_once('../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_GENESIS_PRESTACIONES.p_ibc_prorroga(:v_pnumero,:v_pjson_row); end;');
  oci_bind_by_name($consulta, ':v_pnumero', $request->numeroprorroga);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_pjson_row', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
