...
function subirftp_digital()
{
  ...
  if ($arrlength == 0) {
    ...
    for ($i = 0; $i < $arrlength; $i++) {
      ...
      $name = $data[$i]->codigo . '_' . $tipo_documento . '_' . $documento . '_' . $day;
      ...
      list(, $data[$i]->base) = explode(';', $data[$i]->base); // Proceso para traer el Base64
      list(, $data[$i]->base) = explode(',', $data[$i]->base); // Proceso para traer el Base64
      $base64 = base64_decode($data[$i]->base); // Proceso para traer el Base64
...
function BusquedaRevision()
{
  require_once('../config/dbcon_prod.php');
  global $request;
  $consulta = oci_parse($c, 'begin pq_genesis_digital.p_listar_soporte(:v_pfinicio,:v_pffinal,:v_doc_afil,:v_json_res); end;');
  oci_bind_by_name($consulta, ':v_pfinicio', $request->fechainicio);
  oci_bind_by_name($consulta, ':v_pffinal', $request->fechafin);
  oci_bind_by_name($consulta, ':v_doc_afil', $request->documento);
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_json_res', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
...
function subirftp_afiliacion()
{
  ...
  $path = 'Movilidad/Afiliaciones/' . $hoy;
  if ($arrlength == 0) {
