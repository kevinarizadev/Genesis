...
angular.module('GenesisApp')
    ...
    .controller('consultaautorizacionafiliadosController', ['$scope', '$http', '$location', '$filter', 'ngDialog', '$timeout', 'pqrHttp', 'afiliacionHttp', 'consultaHTTP', function ($scope, $http, $location, $filter, ngDialog, $timeout, pqrHttp, afiliacionHttp,consultaHTTP) {
        ...
        $scope.filter = function (val) {
            ...
            console.log($scope.listarAutorizacionesTemp);
        ...
        $scope.filterSol = function (val) {            
            $scope.listarSolicitudesTemp = $filter('filter')($scope.listarSolicitudes, val);            
            if ($scope.listarSolicitudesTemp.length > 0) {
                $scope.setPageSol(1);
            }
            $scope.configPagesSol();
        }
        $scope.initPaginacionSol = function (info) {
            $scope.listarSolicitudesTemp = info;
            $scope.currentPageSol = 0;
            $scope.pageSizeSol = 10;
            $scope.valmaxpagSol = 10;
            $scope.pagesSol = [];
            console.log($scope.listarSolicitudesTemp.length);
            $scope.configPagesSol();
        }
        $scope.configPagesSol = function () {
            $scope.pagesSol.length = 0;
            var ini = $scope.currentPageSol - 4;
            var fin = $scope.currentPageSol + 5;
            if (ini < 1) {
                ini = 1;
                if (Math.ceil($scope.listarSolicitudesTemp.length / $scope.pageSizeSol) > $scope.valmaxpagSol)
                    fin = 10;
                else
                    fin = Math.ceil($scope.listarSolicitudesTemp.length / $scope.pageSizeSol);
            } else {
                if (ini >= Math.ceil($scope.listarSolicitudesTemp.length / $scope.pageSizeSol) - $scope.valmaxpagSol) {
                    ini = Math.ceil($scope.listarSolicitudesTemp.length / $scope.pageSizeSol) - $scope.valmaxpagSol;
                    fin = Math.ceil($scope.listarSolicitudesTemp.length / $scope.pageSizeSol);
                }
            }
            if (ini < 1) ini = 1;
            ...
            if ($scope.currentPageSol >= $scope.pagesSol.length)
                $scope.currentPageSol = $scope.pagesSol.length - 1;
            if ($scope.currentPageSol < 0) { $scope.currentPageSol = 0; }
        }
        $scope.setPageSol = function (index) {
            $scope.currentPageSol = index - 1;
            if ($scope.pagesSol.length % 2 == 0) {
                var resul = $scope.pagesSol.length / 2;
            } else {
                var resul = ($scope.pagesSol.length + 1) / 2;
            }
            var i = index - resul;
            if ($scope.listarSolicitudesTemp.length % $scope.pageSizeSol == 0) {
                var tamanomax = parseInt($scope.listarSolicitudesTemp.length / $scope.pageSizeSol);
            } else {
                var tamanomax = parseInt($scope.listarSolicitudesTemp.length / $scope.pageSizeSol) + 1;
            }
            var fin = ($scope.pagesSol.length + i) - 1;
            if (fin > tamanomax) {
                fin = tamanomax;
                i = tamanomax - 9;
            }
            if (index > resul) {
                $scope.calcularSol(i, fin);
            }
        }
        $scope.pasoSol = function (tipo) {
            if (tipo == 'next') {
                var i = $scope.pagesSol[0].no + 1;
                if ($scope.pagesSol.length > 9) {
                    var fin = $scope.pagessol[9].no + 1;
                } else {
                    var fin = $scope.pagesSol.length;
                }

                $scope.currentPageSol = $scope.currentPageSol + 1;
                if ($scope.listarSolicitudesTemp.length % $scope.pageSizesol == 0) {
                    var tamanomax = parseInt($scope.listarSolicitudesTemp.length / $scope.pageSizeSol);
                } else {
                    var tamanomax = parseInt($scope.listarSolicitudesTemp.length / $scope.pageSizeSol) + 1;
                ...
                if (i <= 1) {
                    i = 1;
                    fin = $scope.pagesSol.length;
                }
            }
            $scope.calcularSol(i, fin);
        }
        $scope.calcularSol = function (i, fin) {
            if (fin > 9) {
                i = fin - 9;
            } else {
                i = 1;
        ...
        $scope.validarAutorizaciones = function () {
            $scope.nameaut = $scope.check_option_2 ? 'Solicitutes' : 'Autorizaciones';
            $scope.check_option_2 ? $scope.getSolicitudes() : $scope.buscarAutorizaciones()
        }
        ...
        $scope.getSolicitudes = function () {
            ...
            var vdata = { "tipodocumento": $scope.jsonautorizacion.tipodocumento, "documento": $scope.jsonautorizacion.documentoavanzado }
            $http({
                method: 'POST',
                url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
                data: { function: 'p_obtener_solicitud_esaa', data: JSON.stringify(vdata) }
            }).then(function (response) {
                console.log(response.data);                                
                $scope.listarSolicitudes = response.data;
                $scope.initPaginacionSol($scope.listarSolicitudes);
                swal.close();
            });
        }
