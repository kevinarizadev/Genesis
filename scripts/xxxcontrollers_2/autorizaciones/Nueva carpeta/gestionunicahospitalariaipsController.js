...
angular.module('GenesisApp')
  ...
  .controller('gestionunicahospitalariaipsController', ['$scope', '$http', '$location', '$filter', 'ngDialog', '$timeout', 'pqrHttp', 'afiliacionHttp', '$sce', function ($scope, $http, $location, $filter, ngDialog, $timeout, pqrHttp, afiliacionHttp, $sce) {
    ...
    $(document).ready(function () {
      ...
      // var fecha2 = new Date();
      // fecha2.setDate(fecha.getDate() - 50);
      // console.log($scope.tempautunica);
      // $('#idfechaegreso').bootstrapMaterialDatePicker({ format: 'DD/MM/YYYY HH:mm', lang: 'fr', weekStart: 1, nowText: 'Now', minDate:fecha2, maxDate: new Date(), defaultDate: fecha });
    });
...
$scope.Obtener_Tipos_Documentos();
    ...
    $scope.tempautunica = null;
    $scope.selectfecha = null;
    $scope.openmodals = function (tipo, opcion) {
      ...
      switch (tipo) {
        ...
        case 'modalgestionunicahospitalaria':
          ...
          if (document.getElementById('inputFiletextunica')) {
            document.getElementById('inputFiletextunica').value = '';
          }
          if (document.getElementById('epicricisfile')) {
            document.getElementById('epicricisfile').value = '';
          }
          if ($scope.tempautunica.ADJUNTO_EPICRISIS) {
            $scope.viewDocument2($scope.tempautunica.ADJUNTO_EPICRISIS);
          }
          ...
          $('#idfechaegreso').bootstrapMaterialDatePicker({ format: 'DD/MM/YYYY HH:mm', lang: 'fr', weekStart: 1, nowText: 'Now', maxDate: new Date(), defaultDate: fecha, lang: 'es' }).on('change', function (e, date) {
            console.log(fecha2);
            $('#idfechaegreso').bootstrapMaterialDatePicker('setMinDate', fecha2);
          });
          // });
          $("#idfechaegreso").val(output + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds());
          $scope.datetemp.fechasegreso = output + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
    ...
    $scope.buscarAutorizaciones = function () {
      ...
      if ($scope.validatefiltros == false) {
        ...
        $http({
          ...
          data: { function: 'P_OBTENER_SOLICITUD_GESTIONADAS_AVANZADO', autorizacion: JSON.stringify($scope.tempjsonaut) }
        }).then(function (response) {
    ...
    $scope.gestionaraut = function () {
      ...
      // var fechaorden = new Date(formatDateoro($scope.tempautunica.FECHAORDEN));
      // if ($scope.fechasegreso >= fechaorden) {

      console.log($scope.datetemp.fechasegreso);
      ...
      $http({
        ...
        url: "php/autorizaciones/autorizacion/funcautorizacionips.php",
        data: {
          function: 'p_gestiona_egreso_esoa', numero: $scope.tempautunica.NUMERO_AUT, ubicacion: $scope.tempautunica.UBICACION_AUT, fecha: fechaegresotemp,
          tipo_usuario: 'E',
        ...
        if (response.data.Codigo == 0) {
          ...
          $http({
            ...
            data: {
              function: 'subirArchivoAut', file: $scope.epicrisis.file,
              ext: $scope.epicrisis.ext,
              num: $scope.tempautunica.NUMERO_AUT,
              ubicacion: $scope.tempautunica.UBICACION_AUT,
              namefile: 'AUT_GESTIONADA' + $scope.tempautunica.NUMERO_AUT + '_' + $scope.tempautunica.UBICACION_AUT
            } 
          }).then(function (response) {
            function remove_linebreaks(str) {
              return str.replace(/[\r\n]+/gm, "");
            }
            console.log({ 'ARCHIVO': remove_linebreaks(response.data) });

            if ($scope.tempautunica.COD_ADJUNTO_EPICRISIS == "") {
              $scope.subirAdjunto(JSON.stringify({ 'ARCHIVO': remove_linebreaks(response.data) }));
            }
            if ($scope.tempautunica.COD_ADJUNTO_EPICRISIS) {
              $scope.subirAdjuntoUpdate(remove_linebreaks(response.data));
            }
      ...
      //   swal('Importante', 'La fecha de egreso no puede ser menor a la fecha de la orden', 'info');
      // }
    ...
    $scope.subirAdjuntoUpdate = function (vfile) {
      ...
      $http({
        ...
        data: {
          ...
          codadjunto: $scope.tempautunica.COD_ADJUNTO_EPICRISIS,
          ruta: vfile,
          num: $scope.tempautunica.NUMERO_AUT,
          ubicacion: $scope.tempautunica.UBICACION_AUT,
        }
    ...
    $scope.ruta = null;
    $scope.viewDocument2 = function (ruta) {
      $http({
        method: 'POST',
        url: "php/autorizaciones/esoa/esoa.php",
    ...
    $('#epicricisfile').change(function () {//Detecta los cambios que sufre el input file            
      $timeout(function () {//Usado para validar el file en tiempo real
        var file = document.getElementById('epicricisfile').files[0];//Obtiene el file del input para validarlo
        console.log(file);
        $scope.data.formato = '';
          ...
          if (file.size <= 5000000) {//valida que el size del file sea <= 5 mb                                                         
            if (file.name.split('.').pop().toLowerCase() == 'pdf') {//Obtiene la extension del file y valida que sea un pdf                      
              ...
              reader.onload = function (event) {
                $timeout(function () {
                  $scope.epicrisis.file = event.target.result; //Asigna el file al ng-model autFile
                  $scope.epicrisis.ext = file.name.split('.').pop().toLowerCase();//Asigna la extension del autFile
                  $scope.epicrisis.namefile = file.name.substr(0, file.name.lastIndexOf('.'));    //Asigna el nombre del autFile        
                  $scope.epicrisis.pdf = "";
                  $scope.epicrisis.pdf = reader.result;
                })
              };
              reader.readAsDataURL(file);
            } else {
              $scope.data.formato = 'Formato Invalido solo puedes adjuntar archivos con extensión .pdf';
              $scope.epicrisis.file = null;
              $scope.epicrisis.namefile = null;//Asigna null al ng-model autFile  
              $scope.epicrisis.ext = null;//Asigna null a la extension autFile 
