...
if (localStorage.getItem('solicitud_solicitud')) {
  ...
  if (TIPO_DOC && DOCUMENTO && NUMERO) {
    ...
    $scope.buscarAfiliado = function (tipo, tipodocumento, documento) {
      $scope.datosafiliadoscitasautomaticas = [];
      $http({
        method: 'POST',
        ...
        if (response.data.CODIGO != "0") {
          $scope.datosafiliadoscitasautomaticas = response.data;
          // $scope.Agendar_Citas();
          if (tipo == '1') {
                 $http({
              method: 'POST',
      ...
      if ($scope.buscarpro.length   >=3) {
        ...
        $http({
          ...
          data: { function: 'obtenerProducto', 
                  ...
                  edad: edad, 
                  sexo: sexo,
                  tipodocumentoafiliado:$scope.datosafiliadoscitasautomaticas.TipoDocumento,
                  documentoafiliado:$scope.datosafiliadoscitasautomaticas.Documento,
                  // numerodocumento:$scope.datosafiliadoscitasautomaticas.Documento 
                }
        }).then(function (response) {
          swal.close();
          $scope.listProductos = [];
...
$scope.ModalNumeroCelular = function () {
  if(($scope.datosafiliadoscitasautomaticas.EdadAnhos >= 18 && $scope.datosafiliadoscitasautomaticas.EdadAnhos <= 49)
  && $scope.datosafiliadoscitasautomaticas.SexoCodigo == 'F' && 
  ($scope.datosafiliadoscitasautomaticas.Municipio == 'BARRANQUILLA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'SOLEDAD'  || $scope.datosafiliadoscitasautomaticas.Municipio == 'MAGANGUE' ||
  $scope.datosafiliadoscitasautomaticas.Municipio == 'MONTERIA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'RIOACHA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'VALLEDUPAR')){
  $('#Modal_Celular').modal({dismissible: false});
  setTimeout(() => {
    $('#Modal_Celular').modal('open');
  }, 500);
}
}

$scope.Agendar_Citas = () => { 
  var cardnumber = $("#numerocelularminimo").val();
if (cardnumber.length < 10) {
  swal({ title: "Importante", text: "El Numero de celular minimo debe contener 10 Caracteres", type: "info" });
  ...
  ($scope.datosafiliadoscitasautomaticas.Municipio == 'BARRANQUILLA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'SOLEDAD'  || $scope.datosafiliadoscitasautomaticas.Municipio == 'MAGANGUE' ||
  $scope.datosafiliadoscitasautomaticas.Municipio == 'MONTERIA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'RIOACHA' || $scope.datosafiliadoscitasautomaticas.Municipio == 'VALLEDUPAR')){
    var nombreapellido = $scope.datosafiliadoscitasautomaticas.NombreCompleto.split(" ");
    const bodyParameters ={
      tipo_identificacion: $scope.datosafiliadoscitasautomaticas.TipoDocumento,
      identificacion: $scope.datosafiliadoscitasautomaticas.Documento,
      primer_nombre: nombreapellido[0],
      segundo_nombre: nombreapellido[1],
      primer_apellido: nombreapellido[2],
      segundo_apellido: nombreapellido[3],
      genero: $scope.datosafiliadoscitasautomaticas.SexoCodigo,
      fecha_nacimiento: $scope.datosafiliadoscitasautomaticas.FechaNacimiento,
      departamento: $scope.datosafiliadoscitasautomaticas.DEPARTAMENTO,
      ciudad: $scope.datosafiliadoscitasautomaticas.MUNICIPIO,
      departamento_sede: $scope.datosafiliadoscitasautomaticas.DEPARTAMENTO,
      ciudad_sede: $scope.datosafiliadoscitasautomaticas.MUNICIPIO,
      regimen: $scope.datosafiliadoscitasautomaticas.Regimen,
      zona: $scope.datosafiliadoscitasautomaticas.ZONA_NOMBRE,
      celular: $scope.numerodecelular.toString(),
      tienewhatsapp: $scope.cuentaconwhatsapp,
      telefono: $scope.datosafiliadoscitasautomaticas.CELULAR,
      correo_electronico: $scope.datosafiliadoscitasautomaticas.email,
      direccion: $scope.datosafiliadoscitasautomaticas.DIRECCION
    }
    $http({
      method: 'POST',
      url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
      data: {
        ...
        datos: JSON.stringify(bodyParameters)
      }
    }).then(function (response) {
      $('#Modal_Celular').modal('close');
      if(response.data){
        let respuestaapi;
        if(response.data.Status == true){
        respuestaapi = response.data.Data.Mensaje_confirmacion;
        swal({ title: "Cita Agendada Exitosamente", type: "success",position: 'top-end',html:`
        <h5 style="font-family: revert;color:navy"> Doctor: ${response.data.Data.Doctor}</h5>
        <h5 style="font-family: revert;color:navy"> Fecha y Hora Cita:  ${response.data.Data.Fecha_hora}</h5>
        <h5 style="font-family: revert;color:navy"> Paciente: ${response.data.Data.Paciente}</h5>
        <h5 style="font-family: revert;color:navy"> Sede:  ${response.data.Data.Sede}</h5>
        <h5 style="font-family: revert;color:navy"> Direccionde Sede:  ${response.data.Data.Sede_direccion}</p>` });
        }else{
          respuestaapi = "No se Agendo La Cita";
        }
        $http({
          method: 'POST',
          url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
          data: {
            ...
            tipodocumento: $scope.datosafiliadoscitasautomaticas.TipoDocumento,
            numerodocumento: $scope.datosafiliadoscitasautomaticas.Documento,
            datosenviados: JSON.stringify(bodyParameters),
            respuestarecibida: respuestaapi
          }
        }).then(function (response) {
          console.log(response);
        })
    ...
    console.log('no se agendo por que no cumplio las validaciones');
  }
  }
  }
