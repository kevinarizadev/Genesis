...
angular.module('GenesisApp')
  ...
  .controller('autProgramadasController', ['$scope', '$http', '$location', '$filter', 'ngDialog', '$timeout', 'pqrHttp', function ($scope, $http, $location, $filter, ngDialog, $timeout, pqrHttp) {
    ...
    $scope.buscarAfiliado = function (tipo, tipodocumento, documento) {
      $http({
        ...
        if (response.data.CODIGO != "0") {
          $scope.tipodocumentoafiliado=tipodocumento;
          $scope.documentoafiliado=documento;
          if (tipo == '3') {
    ...
    $scope.buscarEspecialidades = function (nitips) {
    //  var nitips = sessionStorage.getItem('nit');
      $http({
        method: 'POST',
        url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
        data: { function: 'obtener_especialidad', nit: nitips }
      }).then(function (response) {
    ...
    $scope.buscarProducto = function (pro) {
      if ($scope.buscarpro.length >= 4) {
        ...
        $http({
          method: 'POST',
          url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
          data: { function: 'obtenerProducto', regimen: regimen, 
                                                contrato: contrato, 
                                                ...
                                                sexo: sexo, 
                                                tipo: 'S',
                                                tipodocumentoafiliado:$scope.tipodocumentoafiliado,
                                                documentoafiliado:$scope.documentoafiliado,
                                              }
        }).then(function (response) {
          $scope.listProductos = [];
          if (response.data["0"].CODIGO == '-1') {
      ...
      swal({
        ...
        if (result) {
          ...
          if ($scope.pasardatosafiliadoprog == true) {
            ...
            if($scope.solicitudpro.altocosto == true) {
              $scope.solicitudpro.altocosto = 'S';
            }else{
              $scope.solicitudpro.altocosto = 'N';
            }
