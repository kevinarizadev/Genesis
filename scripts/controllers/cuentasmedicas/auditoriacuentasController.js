...
angular.module('GenesisApp').controller('auditoriacuentasController', ['$scope', '$http', '$timeout', '$filter', 'ngDialog', 'notify', function ($scope, $http, $timeout, $filter, ngDialog, notify) {
    ...
    $scope.tipodeDocumentos = function () {
        ...
        $http({
            ...
            console.log(data);
            if (data) {
                $scope.listadoTipodocument = data;
    ...
    $scope.Ver_Gestion_Factura = function (X) {
        ...
        $timeout(function () {
            ...
            $scope.HojaGest = {
                ...
                //Actualizacion ps  //
                COPAGO: (X.VALOR_COPAGO == '') ? '0' : $scope.FormatPesoNumero(X.VALOR_COPAGO.replace(',', '.')),
                ALTOCOSTO: (X.ALTO_COSTO2 == 'true') ? true : false,
                NO_COPAGO: (X.MOTIVO_NO_COPAGO == 'S') ? true : false,
                //Actualizacion ps  //
    ...
    $scope.Buscar_Diag = function () {
        console.log('asdla.,smd.mas.dm.amsda');
    ...
    $scope.HojaGestion_Guardar = function () {
        if ($scope.HojaGlosa.Hoja == 1) {
            ...
            if ($scope.HojaGest.ESTADO != 'P') {
                ...
                if($scope.HojaGest.tipoDocument == undefined || $scope.HojaGest.tipoDocument == null || $scope.HojaGest.tipoDocument == ''){
                    Campos_Empty = true; document.querySelector('#tipoDocument_Label').classList.add('red-text');
                }
                                ...
                                if (willDelete) {
                                    ...
                                    $http({
                                        ...
                                        data: {
                                            ...
                                            vptipodoc:$scope.HojaGest.tipoDocument,
                                            vpdocafi:$scope.HojaGest.DOC_AFILIADO_VER,
                                            Diag: $scope.HojaGest.DIAGNOSTICO_COD, Regimen: $scope.HojaGest.REGIMEN,
                                            TipoFac: $scope.HojaGest.TIPO_FACTURA_COD,
                                            Copago: $scope.HojaGest.COPAGO.replace(/\./g, ''),
                                            vpcuotamod:$scope.HojaGest.CUOTA_MODERADORA.replace(/\./g, ''),
                                            ...
                                            vpnocopago: ($scope.HojaGest.NO_COPAGO == true) ? 'S' : 'N',
                                        }
                                    }).then(function (response) {
                                        if (response.data) {
