...
angular.module('GenesisApp')
  .controller('navbarcontroller', ['$scope', 'versionamientoHttp', '$localStorage', 'ngDialog', '$http', '$interval',
    function ($scope, versionamientoHttp, $localStorage, ngDialog, $http, $interval) {
      ...
      $scope.obtenerSession = function () {
        $.getJSON("php/obtenersession.php", dat).done(function (respuesta) {
          ...
          if ($scope.codigorol == "Funcionario") {
            ...
            if ($scope.sesdata.cambioclave == 'N' && document.querySelector("#modalcambioclave") == null) {
              $interval($scope.validarCambiodeClave, 10000); // 30000 milisegundos = 30 segundos
            }
            $scope.validarCambiodeClave()
      ...
      // $scope.abrir();
      console.log($scope.home_user) //inicio

      $scope.validarCambiodeClave = function () {
        $scope.sesdata.cambioclave;
        // if ($scope.home_user == 'inicio') {
        console.log(document.querySelector("#modalcambioclave") == null)
        if ($scope.sesdata.cambioclave == 'S' && document.querySelector("#modalcambioclave") == null) {

          // ngDialog.open({
          //   template: 'views/navegacion/cambioclaveeps.html',
          //   className: 'ngdialog-theme-plain',
          //   controller: 'cambioclaveepscontroller',
          //   scope: $scope,
