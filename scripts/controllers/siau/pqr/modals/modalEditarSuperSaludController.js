...
angular.module('GenesisApp')
  .controller('modalEditarSuperSalud', ['$scope', '$http', '$timeout', 'pqrHttp', function ($scope, $http, $timeout, pqrHttp) {
    ...
    $scope.obtenerTipoDoc = function () {
      ...
      $scope.tiposdocumentos =
        [
          ...
          {
            "CODIGO": "NI",
            "NOMBRE": "NUMERO DE IDENFICACION  TRIBUTARIA",
            "TICN_LMIN": "1",
            "TICN_LMAX": "20"
          }
        ]
    ...
    $scope.obtenerTipoDoc()

    $scope.pqrDataSuper = {
      tipodocumento: $scope.dataPqrExcel.TIPODOCUMENTO,
      documento: $scope.dataPqrExcel.DOCUMENTO,
      nombre_afi: $scope.dataPqrExcel.NOMBRE_AFILIADO,
      estado_afi: $scope.dataPqrExcel.ESTADO_AFILIADO,
    ...
    $scope.searchAfiliado = function () {

      if (!$scope.pqrDataSuper.tipodocumento || !$scope.pqrDataSuper.documento) {
        swal("Mensaje", 'Diligencia tipo y numero de documento', "info").catch(swal.noop);
        return
      }
      swal({
      ...
      $http({
        ...
        data: {
          ...
          tipodocumento: $scope.pqrDataSuper.tipodocumento,
          documento: $scope.pqrDataSuper.documento
        }
      }).then(function ({ data }) {
        swal.close();
        if (data && data.toString().substr(0, 3) != '<br') {
          if (data.NombreCompleto != undefined) {
            $scope.pqrDataSuper.nombre_afi = data.NombreCompleto;
            $scope.pqrDataSuper.estado_afi = data.Estado;
          } else {
    ...
    $scope.validateSave = function () {
      ...
      if ($scope.pqrDataSuper.reporta == 'S') {
        ...
        if (!$scope.pqrDataSuper.nombre_afi) {
          swal('No completado', '¡Afiliado no encontrado, debe validar!', 'error').catch(swal.noop);
          return
        }
