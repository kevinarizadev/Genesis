...
angular.module('GenesisApp')
  .controller('pqrController', ['$scope', '$timeout', 'pqrHttp', 'ngDialog', '$filter', '$http', function ($scope, $timeout, pqrHttp, ngDialog, $filter, $http) {
    ...
    $(document).ready(function () {
      ...
      //   // $("#mediosrecepcion").prop("selectedIndex", 1);
      //   $scope.changeMediosRecepcion("17");
      // }, 3000);
    ...
    $scope.pqrData = {
      ...
      t_seccional: null, clasificacion: null, motivosEspecificos: null, submotivosEspecificos: null, motivoAltoCosto: null, motivoPatologia: null
    ...
    $scope.getPQRS = function (state) {
      ...
      pqrHttp.getPQRS(state, sessionStorage.getItem('cedula')).then(function (response) {
        ...
        if ($scope.cedulausuario == '1140861113') {
          $scope.listadoCompleto = response.sort((a, b) => {
            return b.CODIGO - a.CODIGO;
          });
        } else {
          $scope.listadoCompleto = response
        }


        setTimeout(() => {
          $scope.validarFiltrosTabla()
        }, 300);
        // $scope.listadoNoVencidas = [];
        // $scope.listadoVencidas = [];
        // response.forEach(e => {
        //   if (e.FILTRO_VENCIDA == 'S') {
        //     $scope.listadoVencidas.push(e);
        //   } else {
        //     $scope.listadoNoVencidas.push(e);
        //   }
        // });
        ...
        // setTimeout(() => {
        //   // $scope.pqrs = response;
        //   $scope.pqrs = JSON.parse(JSON.stringify($scope.listadoNoVencidas));
        //   console.log($scope.pqrs.length)
        //   // $scope.pqrs = response;
        //   $scope.current_page = 1;
        //   $scope.arrayPages = [];
        //   $scope.pageActive = 1;
    ...
    $scope.validarFiltrosTabla = function () {
      if (!$scope.listadoCompleto) { return }
      var listado;
      if ($scope.cedulausuario == '1140861113') {
        listado = $scope.listadoCompleto;
      } else {

        $scope.filterPQRS = '';
        // hey

        if ($scope.filterFechaVenc === undefined || $scope.filterFechaVenc == null) { //SIN FECHA
          if ($scope.filterSuperSalud == 'T') {
            // // NOVENCIDA - TODAS
            // // VENCIDA - TODAS
            listado = $scope.listadoCompleto.filter(obj => obj.FILTRO_VENCIDA == $scope.filterVencidas)
          } else {
            // // VENCIDA - SUPER O DERECHO
            // // NOVENCIDA - SUPER O DERECHO
            var listado = $scope.listadoCompleto.filter(obj => obj.FILTRO_VENCIDA == $scope.filterVencidas && obj.FILTRO_SUPER_DERE == $scope.filterSuperSalud)
          }
        }
        if ($scope.filterFechaVenc != undefined) { // CON FECHA
          if ($scope.filterSuperSalud == 'T') {
            // // NOVENCIDA - TODAS
            // // VENCIDA - TODAS
            var listado = $scope.listadoCompleto.filter(obj => obj.FILTRO_VENCIDA == $scope.filterVencidas && obj.FECHAENT == $scope.formatDate2($scope.filterFechaVenc))
          } else {
            // // VENCIDA - SUPER O DERECHO
            // // NOVENCIDA - SUPER O DERECHO
            var listado = $scope.listadoCompleto.filter(obj => obj.FILTRO_VENCIDA == $scope.filterVencidas && obj.FILTRO_SUPER_DERE == $scope.filterSuperSalud && obj.FECHAENT == $scope.formatDate2($scope.filterFechaVenc))
          }
        }
      }
    ...
    $scope.insert = function () {//Hace la request al servidor para insertar el PQR
      ...
      // var fechaparaenviar = new Date($scope.pqrData.fecha_recibido);
      // var formattedDateTime = `${fechaparaenviar.toLocaleDateString()} ${fechaparaenviar.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} ${fechaparaenviar.getHours() >= 12 ? 'PM' : 'AM'}`;
      var fechaparaenviar = new Date($scope.pqrData.fecha_recibido);
      ...
      if (sessionStorage.getItem('cedula')) {
        pqrHttp.postPqr($scope.json, $scope.pqrData.pqrFile, null, 'I', $scope.pqrData.gradfile).then(function (res) {
          // console.log(res);
          if (res.data.Codigo == '1' && ($scope.pqrData.selectedtipoSolicitud == 'D' || $scope.pqrData.selectedtipoSolicitud == 'Q' || $scope.pqrData.selectedtipoSolicitud == 'R') && $scope.pqrServicios.obligAut == 'S' && $scope.pqrData.selectedtipoRadicacion == 'A') {
    ...
    $scope.changeRv = function (riesgo) {//Funcion para validar si aplica o no en riesgo de vida
      const datosselect = JSON.parse(riesgo);
      $scope.pqrData.selectedHoras = datosselect.HORAS;
      $scope.pqrData.selectedRdS = datosselect.CODIGO;
      if (datosselect.CODIGO == 'S') {
    ...
    $scope.resetUser = function () {//Funcion para hacer reset a los datos del Afiliado
      $scope.pqrData.User = {
        ...
        //selectedDocumento: 'CC', documento: '27981738',
        nombre: null, fecha_nacimiento: null,
      ...
      if ($scope.pqrData.User.selectedDocumento && $scope.pqrData.User.documento) {
        ...
        pqrHttp.postSearchAfiliado($scope.dataSearch).then(function (response) {
          ...
          if ($scope.afiliado && $scope.afiliado.NOMBRECOMPLETO) {//Si el nombre existe asigna los datos de la request
            ...
            $scope.pqrData.User.edadDias = $scope.afiliado.EDAD_DIAS;//
            $scope.pqrData.User.sexoCodigo = $scope.afiliado.COD_SEXO;//
      ...
      if ($scope.tabs.tab == 'G') {
        if ($scope.filterPQRS == null || $scope.filterPQRS == '') {
          ...
          if ($scope.cedulausuario != '1140861113') {
            ...
            $scope.filterVencidas = 'T';
            $scope.filterSuperSalud = 'T';
            $scope.filterFechaVenc = undefined;
            $scope.pqrs = JSON.parse(JSON.stringify($scope.listadoCompleto));
            console.log($scope.pqrs.length)
            setTimeout(() => {
              $scope.$apply();
            }, 300);
    ...
    $('#pqrfile').change(function () {//Detecta los cambios que sufre el input file
      $timeout(function () {//Usado para validar el file en tiempo real
        ...
        if (file) {//Valida si existe el archivo en el input file
          if (file.size <= 6242880) {//valida que el size del file sea <= 5 mb
    ...
    $scope.obtenerProductoServicio = function () {
      ...
      $http({
        ...
        data: {
          ...
          // regimen: $scope.pqrData.User.regimen,
          edadDias: $scope.pqrData.User.edadDias,
          sexoCodigo: $scope.pqrData.User.sexoCodigo
        }
      }).then(function ({ data }) {
    ...
    $scope.formatDate2 = function (date) {
      ...
      var d = new Date(date),
        ...
        day = '' + d.getDate(),
        year = d.getFullYear();
      if (year < 2020) { $scope.filterFechaVenc = undefined; return }
      if (month.length < 2) month = '0' + month;
      if (day.length < 2) day = '0' + day;
            ...
            case 'gradfile':
              ...
              if (files.length > 0) {
                if (files[0].name.split('.').pop().toLowerCase() in { 'png': 'png', 'jpg': 'jpg', 'pdf': 'pdf', 'doc': 'doc', 'docx': 'docx' }) {
                  if (files[0].size > 6242880) {//valida que el size del file sea <= 5 mb
                    $timeout(function () {
