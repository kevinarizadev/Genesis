      ...
      $scope.ObtenerRoluser = function () {
        $http({
          ...
          if (response.data && response.data.toString().substr(0, 3) != '<br') {
            ...
            if($scope.rolObtenido == 'SUPERVISOR' && $scope.responsable == '1140871550'){
              setTimeout(() => {
                $scope.Tabs = 7;
                $('#tabs_7').click();
              }, 1000);
        ...
        if ($scope.buscard2 == '' || $scope.buscard2 == null) {
          ...
          $http({
            ...
            data: {
              ...
              ips: $scope.buscard2,
              vpnumeroips: $scope.form3.pnumero ||  $scope.modal3.pnumero || $scope.modal4.pnumero || $scope.modal5.pnumero
            }
          }).then(function (response) {
            // console.log(response);
            // $scope.form3.nombreipsreceptoraVista = response.data[0].NOMBRE;
            if (response.data["0"].Codigo == '0') {
              swal('Importante', 'IPS no encontrada', 'info');
